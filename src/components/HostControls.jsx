...
function TeamRow({ teamId, rank, score, position, active }) {
  ...
  return <div className={`flex items-center gap-3 rounded-2xl border-2 px-3 py-3 ${active ? 'border-[#ff8c4d] bg-[#fff4e8]' : 'border-[#18233f]/15 bg-white'}`}>
    ...
    <div className="flex-1"><p className="font-black text-[#18233f]">Team {TEAM_LABELS[teamId]}</p>{score !== undefined && <p className="text-xs font-semibold text-[#7a8395]">{score} pts{position !== undefined ? ` • Tile ${position + 1} / 30` : ''}</p>}</div>
...
export default function HostControls() {
  ...
  useEffect(() => {
    ...
    const startedAt = game.minigame.startedAt || Date.now();
  ...
  if (game.phase === 'lobby') return <div className="space-y-4">
    <section className="rounded-2xl border-2 border-[#18233f]/15 bg-white p-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-2">{Object.keys(TEAM_LABELS).map((id) => <div key={id} className={`rounded-xl border-2 px-3 py-2 font-bold ${game.teams?.[id] ? 'border-[#4dff79] bg-[#effff2]' : 'border-[#18233f]/10 bg-gray-50 text-gray-400'}`}>Team {TEAM_LABELS[id]}</div>)}</div>
    </section>
    <button onClick={startGame} disabled={!joinedTeams.length} className="w-full rounded-2xl border-4 border-[#18233f] bg-[#ff4d4d] px-5 py-4 font-black text-white shadow-md disabled:opacity-40">🎮 Start Game</button>
    <p className="text-center text-xs font-bold text-[#7a8395]">5 random Rapid Shooting questions will determine dice order.</p>
  </div>;

  if (game.phase === 'minigame') return <div className="space-y-4">
    <section className="rounded-2xl border-2 border-[#18233f]/15 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-wider text-[#ff4d4d]">Rapid Shooting • Dice Order</p><h2 className="mt-1 font-display text-3xl text-[#18233f]">Question {(game.minigame?.questionIndex ?? 0) + 1} / 5</h2><p className="mt-3 rounded-xl bg-[#fff8e7] p-4 font-black text-[#18233f]">Teams are answering. The next question appears automatically.</p></section>
    <section className="rounded-2xl border-2 border-[#18233f]/15 bg-white p-4"><p className="mb-3 text-xs font-black uppercase tracking-wider text-[#7a8395]">Live scores</p><div className="space-y-2">{Object.entries(scores).sort(([, a], [, b]) => b - a).map(([teamId, score], index) => <TeamRow key={teamId} teamId={teamId} rank={index + 1} score={score} />)}</div></section>
  </div>;
