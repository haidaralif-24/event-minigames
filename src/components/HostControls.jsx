...
export default function HostControls() {
  ...
  if (game.phase === 'opening-results' || game.phase === 'turn-order') return <div className="space-y-4"><section className="rounded-2xl border-2 border-[#18233f]/10 bg-white p-5 text-center"><p className="text-xs font-black uppercase tracking-wider text-[#ff8c4d]">Opening order</p><h2 className="mt-1 font-display text-3xl text-[#18233f]">Team {TEAM_LABELS[game.turnOrder?.[0]]} first</h2><p className="mt-2 text-sm font-semibold text-[#7a8395]">Starting the 2-dice race…</p></section>{(game.turnOrder || []).map((id, index) => <TeamRow key={id} teamId={id} rank={index + 1} score={game.opening?.scores?.[id] || 0} />)}</div>;
  if (game.phase === 'minigame') return <div className="space-y-4"><QuestionPanel game={game} /><section className="rounded-2xl border-2 border-[#18233f]/10 bg-white p-4"><p className="mb-3 text-xs font-black uppercase tracking-wider text-[#7a8395]">Next-round race</p><div className="space-y-2">{Object.entries(game.minigame?.scores || {}).sort(([, a], [, b]) => b - a).map(([id, score], index) => <TeamRow key={id} teamId={id} rank={index + 1} score={score} />)}</div></section></div>;
  if (game.phase === 'minigame-results' || game.phase === 'round-transition') return <div className="space-y-4"><section className="rounded-2xl border-2 border-[#18233f]/10 bg-white p-5 text-center"><p className="text-xs font-black uppercase tracking-wider text-[#ff8c4d]">Round {game.round} complete</p><h2 className="mt-1 font-display text-3xl text-[#18233f]">Next dice order</h2></section>{(game.rankings || []).map((r) => <TeamRow key={r.teamId} teamId={r.teamId} rank={r.position} score={game.minigame?.scores?.[r.teamId] || 0} position={positions[r.teamId] ?? 0} />)}</div>;
  if (game.phase === 'finished') return <div className="space-y-4"><section className="rounded-2xl border-2 border-[#18233f]/10 bg-white p-5 text-center"><div className="text-5xl">🏆</div><p className="mt-2 text-xs font-black uppercase tracking-wider text-[#ff8c4d]">Winner</p><h2 className="font-display text-4xl text-[#18233f]">Team {TEAM_LABELS[game.winner]}</h2></section>{(game.rankings || []).map((r) => <TeamRow key={r.teamId} teamId={r.teamId} rank={r.position} position={positions[r.teamId] ?? 0} />)}<button onClick={() => resetForNewGame().catch(console.error)} className="w-full rounded-2xl border-4 border-[#18233f] bg-[#4d79ff] px-5 py-4 font-black text-white">NEW GAME</button></div>;
  return <div className="space-y-4"><section className="rounded-2xl border-2 border-[#18233f]/10 bg-white p-4"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-[#7a8395]">Round {game.round}</p><h2 className="font-display text-3xl text-[#18233f]">2-Dice Race</h2></div><span className="rounded-full bg-[#4dff79] px-3 py-1 text-xs font-black text-[#18233f]">LIVE</span></div><div className="mt-4 space-y-2">{leaderboard.map((teamId, index) => <TeamRow key={teamId} teamId={teamId} rank={index + 1} position={positions[teamId] ?? 0} active={game.activeTeamId === teamId} />)}</div></section><section className="rounded-2xl bg-[#18233f] p-4 text-center text-white"><p className="text-xs font-black uppercase tracking-wider text-white/60">Current turn</p><p className="mt-1 font-display text-3xl">Team {TEAM_LABELS[game.activeTeamId]}</p><p className="mt-1 text-sm font-bold text-white/70">Players roll both dice from their own device.</p></section></div>;
}
