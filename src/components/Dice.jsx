...
/**
 * Dice renders a SINGLE die (a 1-die race). It mirrors the active player's
 * roll across all clients via the `rolling`/`value` props coming from
 * Firestore, so host, spectator board, and other players see the same synced
 * animation.
 ...
 * - read-only (no onRollStart): purely reflects `rolling`/`value`, used on
 *   the host and spectator board to mirror the active player's roll.
 */
export default function Dice({ sides = 6, rolling = false, value = 1, onRollStart, disabled = false }) {
