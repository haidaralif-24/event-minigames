  ...
  return <PlayerShell title={room.phase === 'rapid-shot' ? 'Rapid Shot' : room.phase === 'challenge' ? 'Challenge Tile' : 'Player Console'} step={room.phase} me={me} room={room}>
    ...
    {room.phase === 'board' && <section className="mx-auto max-w-4xl text-center">
      ...
      <div className="rounded-3xl bg-white px-10 py-12 shadow-xl ring-1 ring-black/5">
        {activeId === session.playerId ? <><div className="mx-auto grid h-24 w-24 place-items-center rounded-3xl bg-[#ff8c4d] text-5xl shadow-lg">🎲</div><h1 className="mt-6 text-5xl font-black">Your turn!</h1><p className="mt-3 text-lg font-bold text-[#737887]">Roll the dice to move on the projected board.</p><div className="mt-8 flex justify-center"><Dice onRoll={roll} /></div></> : <><div className="text-7xl">👀</div><h1 className="mt-6 text-5xl font-black">{players[activeId]?.name}'s turn</h1><p className="mt-3 text-lg font-bold text-[#737887]">Watch the projected board. Your turn is coming up!</p></>}
    ...
    {room.phase === 'finished' && <section className="mx-auto max-w-4xl text-center"><PhaseLabel step="FINISH">Final results</PhaseLabel><div className="rounded-3xl bg-white px-10 py-12 shadow-xl ring-1 ring-black/5"><div className="text-7xl">🏆</div><h1 className="mt-5 text-5xl font-black">Game complete!</h1><div className="mx-auto mt-8 max-w-2xl space-y-3">{placements.map((id, index) => <div key={id} className={`flex items-center rounded-2xl px-6 py-5 text-left ${index === 0 ? 'bg-[#fff3c4]' : 'bg-[#f4f5f8]'}`}><span className="mr-5 text-3xl">{['🥇', '🥈', '🥉'][index]}</span><span className="flex-1 text-xl font-black">{players[id]?.name}</span><span className="font-black text-[#6c7180]">Tile {(room.boardPositions?.[id] || 0) + 1}</span></div>)}</div><p className="mt-8 font-black text-[#4d79ff]">Great game, {me?.name}!</p></div></section>}
