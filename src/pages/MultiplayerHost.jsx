    ...
    {(room.phase === 'rapid-shot' || room.phase === 'minigame') ? (
      ...
      <main className="mx-auto grid max-w-[1520px] items-start gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <aside>
          ...
          {room.phase === 'board' && <section className="mb-4 rounded-[20px] bg-[#fff8e7] p-[18px] text-[#18233f]"><h2 className="mb-3 font-display text-base">This Turn</h2><div className="flex items-center gap-3"><Dice rolling={Boolean(room.rolling?.playerId)} die1={room.lastRoll?.die1} die2={room.lastRoll?.die2} value={room.lastRoll?.value || 1} /><div><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#ff8c4d]">{room.rolling?.playerId ? 'Rolling now' : 'Up next'}</p><p className="text-base font-black">{players[activeId]?.name || '—'}</p><p className="text-xs font-extrabold text-[#7a8395]">{room.lastRoll?.value ? `Rolled ${room.lastRoll.die1} + ${room.lastRoll.die2} = ${room.lastRoll.value}` : ''}</p></div></div></section>}
